...
static void hideSubContents(GtkWidget *widget, gpointer **data);
...
static void registrationForm(GtkWidget *widget, gpointer test);
...
static void changePage2(int *page, gpointer data);
static void show(GtkWidget *objet, gpointer data);
...
static void hide(GtkWidget *objet, gpointer data){
...
static void mainMenu(GtkApplication* app, gpointer data){
    ...
    GtkWidget **button = malloc(sizeof(GtkWidget *) * 5);
    ...
    test.window = gtk_application_window_new(app);
    gtk_window_set_position(GTK_WINDOW(test.window), GTK_WIN_POS_CENTER);
    gtk_window_set_default_size(GTK_WINDOW(test.window), 1280, 720);
    gtk_container_set_border_width(GTK_CONTAINER(test.window), 30);
    gtk_window_set_title(GTK_WINDOW(test.window), "PARK'CAR");
    gtk_widget_modify_bg(test.window, GTK_STATE_NORMAL, &color);

    printf("I created the window\n");

    //SCROLLBAR
    scrollbar = gtk_scrolled_window_new(NULL, NULL);
    gtk_container_add(GTK_CONTAINER(test.window),scrollbar);
    test.vbox = gtk_vbox_new(FALSE, 0);
    gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(scrollbar), test.vbox);
    ...
    gtk_box_pack_start(GTK_BOX(test.vbox), toolbar, FALSE, FALSE, 5);
    ...
    if(test.idPage == 1){
        ...
        gtk_box_pack_start(GTK_BOX(test.vbox), headerTitleLabel, FALSE, FALSE, 0);

        frame = gtk_frame_new("Discover the PARK'CAR service");
        gtk_box_pack_start(GTK_BOX(test.vbox), frame, FALSE, FALSE, 5);
        ...
        gtk_box_pack_start(GTK_BOX(test.vbox), buttonArray, FALSE, FALSE, 20);
        ...
        //g_signal_connect(G_OBJECT(button[0]), "clicked", G_CALLBACK(registrationForm), &test);
        g_signal_connect(G_OBJECT(button[0]), "clicked", G_CALLBACK(hide), vBoxFrame);
        g_signal_connect(G_OBJECT(button[0]), "clicked", G_CALLBACK(hide), frame);
        g_signal_connect(G_OBJECT(button[0]), "clicked", G_CALLBACK(hideSubContents), button);


        if(!gtk_widget_get_visible(button[4])){
            test.idPage = 2;
            printf("\nidpage: %d", test.idPage);
        }
    ...
    }else if(test.idPage == 2){
        printf("\nJe suis dans la page 2");
        ...
        gtk_box_pack_start(GTK_BOX(test.vbox), formArray, FALSE, FALSE, 0);
        printf("mm?");
        //NAME
...
static void hideSubContents(GtkWidget *widget, gpointer **data){
    ...
    gtk_widget_hide(data[1]);
    gtk_widget_hide(data[2]);
    gtk_widget_hide(data[3]);
    gtk_widget_hide(data[4]);
...
static void registrationForm(GtkWidget *widget, gpointer test){
    ...
    struct MyWidgets *objet = (MyWidgets *)test;
    /*GtkWidget *window;*/
    GtkWidget *vbox;
    ...
    //DISPLAY THE WINDOW
    gtk_widget_show_all(objet->window);
    printf("window");
